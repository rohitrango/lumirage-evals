...
from vfa.models.timer import RegistrationTimer
...
class VFA(BaseModel):
    ...
    def forward_patches(self, sample):
        f_img = sample['f_img']
        m_img = sample['m_img']
        H, W, D = f_img.shape[2:]
        patch_size = (self.params['model']['in_shape'])[-3:]
        stride = [x//2 for x in patch_size]
        num_patches = [int(np.ceil((dim - ps)*1.0/st + 1)) for dim, ps, st in zip([H, W, D], patch_size, stride)]
        print(f"Running forward for {num_patches[0]}x{num_patches[1]}x{num_patches[2]} patches")

        print(f"f_img.min(), f_img.max() = {f_img.min()}, {f_img.max()}")
        print(f"m_img.min(), m_img.max() = {m_img.min()}, {m_img.max()}")

        displacement = torch.zeros(1, 3, H, W, D)
        counts = torch.zeros(1, 1, H, W, D)
        id_grid_like = None
        ...
        with RegistrationTimer(gpu_sampling_rate=0.05) as timer:
            for i, j, k in tqdm(np.ndindex(*num_patches), total=num_patches[0]*num_patches[1]*num_patches[2]):
                start_idx = [min(i*stride[0], H-patch_size[0]), min(j*stride[1], W-patch_size[1]), min(k*stride[2], D-patch_size[2])]
                end_idx = [start_idx[0] + patch_size[0], start_idx[1] + patch_size[1], start_idx[2] + patch_size[2]]

                F_patch = f_img[:, :, start_idx[0]:end_idx[0], start_idx[1]:end_idx[1], start_idx[2]:end_idx[2]]
                M_patch = m_img[:, :, start_idx[0]:end_idx[0], start_idx[1]:end_idx[1], start_idx[2]:end_idx[2]]
                F = self.encoder(F_patch.to(self.device))
                M = self.encoder(M_patch.to(self.device))
                composed_grids = self.decoder(F, M)

                # get displacement field out of last composed grid
                composed_grid = composed_grids[-1].cpu()
                if id_grid_like is None:
                    id_grid_like = identity_grid_like(composed_grid, normalize=False)
                disp = composed_grid - id_grid_like

                # add it to the displacement field
                displacement[:, :, start_idx[0]:end_idx[0], start_idx[1]:end_idx[1], start_idx[2]:end_idx[2]] += disp
                counts[:, :, start_idx[0]:end_idx[0], start_idx[1]:end_idx[1], start_idx[2]:end_idx[2]] += 1
            torch.cuda.synchronize()
        
        print(f"counts.min(), counts.max() = {counts.min()}, {counts.max()}")
        assert counts.min() > 0, "Some patches were empty"
        # get timer results
        stats = timer_results = timer.stop()
        print("Timer results:")
        print(f"Wall time: {stats['wall_time']:.2f} seconds")
        print(f"Mean GPU utilization: {stats['gpu_util_mean']:.1f}%")
        print(f"Max GPU utilization: {stats['gpu_util_max']:.1f}%")
        print(f"Mean GPU memory: {stats['gpu_memory_mean']:.1f} MB")
        print(f"Max GPU memory: {stats['gpu_memory_max']:.1f} MB")
        print()
        results = {
            'timer': timer_results,
        }
        # get sample size
        displacement = displacement / counts
        id_grid_large = identity_grid_like(displacement, normalize=False)
        print(f"initialized id_grid_large of size {id_grid_large.shape}, min,max = {id_grid_large.min()}, {id_grid_large.max()}")
        warp = id_grid_large + displacement

        # w_img = grid_sampler(sample['m_img'].to('cpu'), warp)
        # results.update({'w_img': w_img})
        # results = self.generate_results(warp, sample)
        results.update({'grid': warp})
        results.update({'composed_grids': [warp],

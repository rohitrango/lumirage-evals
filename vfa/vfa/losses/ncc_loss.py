...
import numpy as np
import math
from fireants.losses.fusedcc import FusedLocalNormalizedCrossCorrelationLoss as FusedNCC
...
class NCC(torch.nn.Module):
    ...
    def  __init__(self, **kwargs):
        ...
        self.use_fused = kwargs['use_fused'] if 'use_fused' in kwargs else False
        print(f"NCC: {self.num_scales}, {self.kernel}, {self.half_resolution}, {self.use_fused}")

        self.similarity_metric = []
        for i in range(self.num_scales):
            print(f"window_size: {kwargs['window_size']-(i*2)}")
            self.similarity_metric.append(
                        SingleScaleNCC(kwargs['window_size']-(i*2)) if not self.use_fused else FusedNCC(3, kernel_size=kwargs['window_size']-(i*2), use_ants_gradient=True, use_separable=True)
            )

...
class BaseDataset(Dataset, ABC):
    ...
    class ClassMask(object):
        def __call__(self, sample):
            if 'f_seg' in sample:
                sample['f_img'] = sample['f_img'] * (sample['f_seg'] > 0)
            if 'm_seg' in sample:
                sample['m_img'] = sample['m_img'] * (sample['m_seg'] > 0)
            return sample
    ...
    class AdjustShape(object):
        ...
        def adjust_to_target_shape(self, arr, key=None):
            ...
            if 'seg' in key or 'mask' in key:
                sum_adjusted = (adjusted > 0).sum()
                sum_original = (arr > 0).sum()
                reduction_ratio = sum_adjusted / sum_original
                if sum_adjusted != sum_original:
                    print(f"⚠️ WARNING: {key} has {reduction_ratio} reduction ratio from {sum_original} to {sum_adjusted}")
        ...
        def __call__(self, sample):
            ...
            for key in sample:
                if key in ['f_img', 'm_img', 'f_seg', 'm_seg', 'f_mask', 'm_mask', 'f_input', 'm_input']:
                    sample[key] = self.adjust_to_target_shape(sample[key], key)
            return sample
        ...
        def __call__(self, sample):
            for key in sample:
                if key in ['f_img', 'm_img']:
                    ...
                    sample[key] = self.norm(sample[key] )
            return sample
    ...
    # class PadByOne(object):
    #     # this is called on tensors
    #     def __call__(self, sample):
    #         for key in sample:
    #             if key in ['f_img', 'm_img', 'f_seg', 'm_seg', 'f_mask', 'm_mask', 'f_input', 'm_input']:
    #                 sample[key] = torch.nn.functional.pad(sample[key], ((0, 1), (0, 1), (0, 1)), mode='constant', value=0)
    #         return sample
